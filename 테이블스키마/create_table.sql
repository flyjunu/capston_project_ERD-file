...
-- 21-1. 직종별 게시판 (답글) 테이블   - 12-01(대댓글 기능 추가)
CREATE TABLE job_community_board_answer (
    ...
    FOREIGN KEY (parent_answer_id) REFERENCES job_community_board_answer(answer_id) ON DELETE CASCADE -- 자기 자신 참조해서 부모댓글 삭제시 자식도 삭제
);

/*
-- 22-2. 직종별 게시판 답글(댓글) 좋아요 기록 테이블 
CREATE TABLE job_community_board_answer_like (
    like_id     NUMBER(20) GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, // 
    answer_id   NUMBER(20) NOT NULL,
    user_id     NUMBER(20) NOT NULL,
    created_at  TIMESTAMP DEFAULT SYSTIMESTAMP,
    CONSTRAINT fk_al_answer FOREIGN KEY (answer_id) REFERENCES job_community_board_answer(answer_id) ON DELETE CASCADE,
    CONSTRAINT fk_al_user   FOREIGN KEY (user_id) REFERENCES users_t(user_id) ON DELETE CASCADE,
    CONSTRAINT uq_answer_like_unique UNIQUE (answer_id, user_id)
);
-- insert 문 예시, 시퀸스 사용X 자동 관리 사용.
-- INSERT INTO job_community_board_answer_like (answer_id, user_id) VALUES (10, 500);
*/

-- 22.3 직종별 게시판 글(본문)좋아요 기능 //12-06 추가
CREATE TABLE JOB_COMMUNITY_BOARD_LIKE (
    LIKE_ID     NUMBER(20) GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    POST_ID     NUMBER(20) NOT NULL,
    USER_ID     NUMBER(20) NOT NULL,
    CREATED_AT  TIMESTAMP DEFAULT SYSTIMESTAMP,
    CONSTRAINT FK_JBL_POST FOREIGN KEY (POST_ID) REFERENCES JOB_COMMUNITY_BOARD(POST_ID) ON DELETE CASCADE,
    CONSTRAINT FK_JBL_USER FOREIGN KEY (USER_ID) REFERENCES USERS_T(USER_ID) ON DELETE CASCADE,
    CONSTRAINT UQ_BOARD_LIKE_UNIQUE UNIQUE (POST_ID, USER_ID)
);
