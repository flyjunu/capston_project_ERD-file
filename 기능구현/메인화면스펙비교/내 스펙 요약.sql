...
-- 2. 자격증 백분율 조회
SELECT cert_total_score,
       ROUND(CUME_DIST() OVER (ORDER BY cert_total_score DESC) * 100, 1) AS upper_percent
...
-- 3. 인턴경험 백분율 조회
SELECT intern_count,
       ROUND(CUME_DIST() OVER (ORDER BY intern_count DESC) * 100, 1) AS upper_percent
...
SELECT intern_count, ROUND(CUME_DIST() OVER (ORDER BY intern_count DESC) * 100, 1) AS upper_percent
FROM (SELECT u.user_id, NVL(i.cnt, 0) AS intern_count
      FROM users_t u, (SELECT ua.user_id, COUNT(*) AS cnt
                       FROM user_activities ua
                       WHERE ua.activity_type IN ('인턴', '인턴십')
