-- 1. 공모전 백분율 조회
SELECT A.contest_count as 공모전참여수, A.백분율
FROM (SELECT x.user_id, x.contest_count,
             ROUND(CUME_DIST() OVER (ORDER BY x.contest_count DESC) * 100, 1) AS 백분율
...
-- 2. 자격증 백분율 조회
SELECT 자격증_점수, 백분율
FROM (SELECT x.user_id, x.cert_total_score AS 자격증_점수,
      ROUND(CUME_DIST() OVER (ORDER BY x.cert_total_score DESC) * 100, 1) AS 백분율
      FROM (SELECT u.user_id, NVL(s.total_score, 0) AS cert_total_score
            FROM users_t u, (SELECT uci.user_id, SUM((CASE c.certificate_category
                                                      ...
                                                      WHEN '기술사'  THEN 5
                                                      ELSE 0 END) + 
                                                      (CASE WHEN c.pass_rate < 10 THEN 5
                              ...
                              AND uci.certificate_id <> 576 -- 576(토익) 제외
                              GROUP BY uci.user_id) s
            WHERE u.user_id = s.user_id(+)) x
       ) all_percentiles
WHERE all_percentiles.user_id = :user_id;   

-- 3. 인턴경험 백분율 조회
SELECT 인턴횟수, 백분율
FROM (SELECT x.user_id, x.intern_count AS 인턴횟수,
             ROUND(CUME_DIST() OVER (ORDER BY x.intern_count DESC) * 100, 1) AS 백분율
...
SELECT 활동횟수, 백분율
FROM (SELECT x.user_id, x.activity_count AS 활동횟수,
             ROUND(CUME_DIST() OVER (ORDER BY x.activity_count DESC) * 100, 1) AS 백분율
...
-- 5. 토익 기준 백분율 조회
SELECT toeic_score, upper_percent
FROM (SELECT x.user_id, x.toeic_score,
             ROUND(CUME_DIST() OVER (ORDER BY x.toeic_score DESC) * 100, 1) AS upper_percent
