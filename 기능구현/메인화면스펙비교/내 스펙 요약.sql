-- 1. 공모전 백분율 조회
        ...
        FROM (SELECT x.user_id,
                     ROUND(CUME_DIST() OVER (ORDER BY x.contest_count) * 100, 1) AS percentile
...
-- 2. 자격증 백분율 조회
        SELECT A.percentile
        FROM (SELECT x.user_id,
                     ROUND(CUME_DIST() OVER (ORDER BY x.cert_total_score) * 100, 1) AS percentile
...
-- 3. 인턴경험 백분율 조회
        SELECT A.percentile
        FROM (SELECT x.user_id,
                     ROUND(CUME_DIST() OVER (ORDER BY x.intern_count) * 100, 1) AS percentile
...
-- 4. 활동 경험 백분율 조회 (인턴 뺴고 나머지 활동) 
        SELECT A.percentile
        FROM (SELECT x.user_id,
                     ROUND(CUME_DIST() OVER (ORDER BY x.activity_count) * 100, 1) AS percentile
              FROM (SELECT u.user_id, NVL(i.cnt, 0) AS activity_count
                    FROM users_t u, ( SELECT ua.user_id, COUNT(*) AS cnt
                                      FROM user_activities ua
                                      WHERE ua.activity_type NOT IN ('인턴', '인턴십', '공모전')
                                      GROUP BY ua.user_id) i
                   WHERE u.user_id = i.user_id(+)) x
               ) A
        WHERE A.user_id = :user_id
...
-- 5. 토익 기준 백분율 조회
        SELECT A.percentile
        FROM (SELECT x.user_id,
                     ROUND(CUME_DIST() OVER (ORDER BY x.toeic_score) * 100, 1) AS percentile
